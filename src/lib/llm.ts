 ...
 * Gateway가 없거나 실패하면 룰/템플릿으로 즉시 폴백한다.
 */
const MODEL = process.env.AI_REVIEW_MODEL ?? "google/gemini-2.5-flash-lite";
const FALLBACK_MODEL = process.env.AI_REVIEW_FALLBACK_MODEL ?? "deepseek/deepseek-v4.1-flash";
export const llmEnabled = !!(process.env.AI_GATEWAY_API_KEY || process.env.VERCEL_OIDC_TOKEN || process.env.VERCEL);
  ...
  try {
    return await cached<ReviewVerdict>("review", key, async () => {
      const result = await generateText({
        ...
        providerOptions: { gateway: { models: [FALLBACK_MODEL], tags: ["feature:scam-review", "tier:mvp"] } },
      });
      return { ...result.output, model: MODEL, costUsd: gatewayCost(result.providerMetadata) };
    });
  } catch {
  ...
  try {
    return await cached<EvidenceImageVerdict>("evidence-image", key, async () => {
      const result = await generateText({
        ...
        providerOptions: { gateway: { models: [FALLBACK_MODEL], tags: ["feature:evidence-image", "tier:mvp"] } },
      });
      return { ...result.output, model: MODEL, costUsd: gatewayCost(result.providerMetadata) };
    });
  } catch {
  ...
  try {
    return await cached("summary", key, async () => {
      const result = await generateText({
        ...
        providerOptions: { gateway: { models: [FALLBACK_MODEL], tags: ["feature:account-summary", "tier:mvp"] } },
      });
      return { summary: result.text.trim() || templateSummary(input), model: MODEL };
    });
  } catch {
