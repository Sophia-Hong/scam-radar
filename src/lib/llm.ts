 ...
 *  2) HIGH 계정의 '범죄사실요약' 생성 — 계정당 1회, 내용 해시로 캐시
...
export async function reviewPost(text: string, reasonLabels: string[]): Promise<ReviewVerdict> {
  ...
  return cached<ReviewVerdict>("review", key, async () => {
    ...
    const msg = await client().messages.create({
      model: MODEL,
      max_tokens: 200,
      system: "너는 한국 SNS의 '주식·코인 리딩방 유인 사기' 탐지 검수자다. 게시물이 (a) 텔레그램·카톡 등 외부 메신저로 유도하면서 (b) 투자 수익을 미끼로 삼는 '유인글'인지 판단한다. 단순 투자 잡담, 뉴스 공유, 개인 후기는 benign. JSON 만 출력: {\"verdict\":\"scam|benign|unsure\",\"confidence\":0~1,\"rationale\":\"한 문장\"}",
...
export async function summarizeAccount(input: SummaryInput): Promise<{ summary: string; model: string }> {
  ...
  return cached("summary", key, async () => {
    ...
    const msg = await client().messages.create({
      model: MODEL,
      max_tokens: 500,
      system: "너는 사이버범죄 신고서의 '범죄사실 요약'을 쓰는 보조자다. 판단이나 추측 없이, 수집된 증거에서 확인되는 요건사실만 육하원칙으로 3~5문장, 존댓말 없이 개조식으로 쓴다. 형식: 1) 피신고 계정 2) 게시 일시·장소(URL) 3) 게시 내용(유인 문구 인용) 4) 유도 수단(메신저·링크) 5) 반복성. 법조문 인용 금지, '사기'라는 단정 대신 '유인 정황'으로 표현.",
...
export function templateSummary(i: SummaryInput): string {
  ...
  return [
    `1) 피신고 계정: ${i.platform} "${i.handle}" (${i.profileUrl})`,
